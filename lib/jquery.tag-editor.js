    ...
    return this.each(function() {
      ...
      function createTag(tag, skipOnChange) {
        ...
        var $tagElement = $(`<div class="tag">
          ...
          <span class="tag-delete ${config.canDelete(tag) ? '' : 'hidden'}"><i class="fa fa-times"></i></span>
        </div>`);
        $tagElement.find('.tag-content')[0].innerText = tag;
        $(config.selector.tags).append($tagElement);
