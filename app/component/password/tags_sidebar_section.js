...
var TagSidebarSectionComponent = SecondarySidebarSectionComponent.extend('passbolt.component.password.TagSidebarSection', /** @static */ {
	...
	_initTree: function() {
		...
		var tree = new TreeComponent('#js_rs_details_tags_list', {
	 ...
	 */
	_initForm: function() {
		var slugs = this.options.resource.tags.attr()
		...
		// Init the form
		var formHtml = View.render(tagUpdateFormTemplate);
	...
	_initAutocomplete: function(tags) {
		var isAdmin = this.options.resource.permission.isAllowedTo(PermissionType.ADMIN);
		...
		var autocomplete = new autoComplete({
			...
			source: (term, suggest) => {
				...
				var matches = slugs.filter(item => item.toLowerCase().indexOf(term) != -1);
				suggest(matches);
			}
		});
	},
	 ...
	 * Cancel the changes
	 */
	' #js_tags_editor_cancel click': function() {
		this._destroyForm();
	},
	 ...
	 */
	' #js_edit_tags_button click': function(el, ev) {
